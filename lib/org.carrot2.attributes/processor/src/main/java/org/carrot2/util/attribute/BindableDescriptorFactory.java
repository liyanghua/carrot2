...
/**
 * A factory for accessing {@link IBindableDescriptor} for a given class.
 * 
 * @see #getDescriptor(Class)
 */
public final class BindableDescriptorFactory
{
    private BindableDescriptorFactory()
    {
        // no instances.
    }

    /**
     * Return the mapped descriptor class name for a given class name (descriptors for
     * nested classes become top-level classes).
     */
    public static String getDescriptorClassName(String className)
    {
        String packageName = getPackageName(className);

        // We apply getShortClassName() twice; the first call returns .-separated class
        // nesting,
        ...
        return packageName + (isEmpty(packageName) ? "" : ".") + shortClassName
            + "Descriptor";
    }
    ...
    {
        if (clazz.getAnnotation(Bindable.class) == null) throw new IllegalArgumentException(
            "Class not marked with @Bindable: " + clazz.getName());

        ClassLoader cl = clazz.getClassLoader();
        String descriptorClassName = getDescriptorClassName(clazz.getName());
        try
        {
            return (Class<? extends IBindableDescriptor>) Class.forName(
                descriptorClassName, true, cl);
        }
